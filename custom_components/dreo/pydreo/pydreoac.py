...
class PyDreoAC(PyDreoBaseDevice):
    ...
    @mode.setter
    def mode(self, mode: ACMode) -> None:
    ...
    @fan_mode.setter
    def fan_mode(self, mode: ACFanMode) -> None:
        """Set fan mode if requested"""
        _LOGGER.debug("PyDreoAC:fan_mode.setter(%s) %s --> %s",
                      self.name, self._fan_mode, mode)
        self._fan_mode = mode
    ...
    def target_temperature(self, value: int) -> None:
        ...
        _LOGGER.debug("PyDreoAC:target_temperature.setter(%s) %s --> %s",
                      self, self._target_temperature, value)
    ...
    def target_humidity(self, value: int) -> None:
        ...
        _LOGGER.debug("PyDreoAC:target_humidity.setter(%s) %s --> %s",
                      self, self._target_humidity, value)
    ...
    def oscon(self, value: bool) -> None:
        ...
        _LOGGER.debug("PyDreoAC:oscon.setter(%s) -> %s (%s)",
                      self.name, value, set_val)
    ...
    def childlockon(self, value: bool) -> None:
        ...
        _LOGGER.debug("PyDreoAC:childlockon.setter(%s) --> %s",
                      self.name, value)
    ...
    def panel_sound(self, value: bool) -> None:
        ...
        _LOGGER.debug("PyDreoAC:panel_sound.setter(%s) --> %s",
                      self.name, value)
    ...
    def preset_mode(self, mode: str) -> None:
        """Set the preset mode."""
        _LOGGER.debug("PyDreoAC:preset_mode.setter(%s) %s --> %s",
                      self.name, self._preset_mode, mode)

        if mode == PRESET_ECO:
            self._send_command(MODE_KEY, ACMode.ECO)
        else:
            self._send_command(MODE_KEY, ACMode.COOL)
    ...
    def update_state(self, state: dict):
        ...
        self._target_humidity = self.get_state_update_value(
            state, TARGET_HUMIDITY_KEY)
        self.work_time = self.get_state_update_value(state, WORK_TIME)
        self.temp_target_reached = "Yes" if self.get_state_update_value(
            state, TEMP_TARGET_REACHED) > 0 else "No"
        # TODO ecopauserate

    def handle_server_update(self, message):
        """Process a websocket update"""
        _LOGGER.debug("PyDreoAC:handle_server_update(%s): %s",
                      self.name, message)
        ...
        if isinstance(val_poweron, bool):
            ...
            _LOGGER.debug(
                "PyDreoAC:handle_server_update - poweron is %s", self._is_on)
        ...
        if isinstance(val_target_temperature, int):
            _LOGGER.debug("PyDreoAC(%s):handle_server_update - target_temperature: %s --> %s",
                          self, self._target_temperature, val_target_temperature)
        ...
        if isinstance(val_mode, int):
            ...
            if target_mode == 5:
                target_mode = ACMode.COOL
                self._preset_mode = PRESET_ECO
            else:
                self._preset_mode = PRESET_NONE
            _LOGGER.debug(
                "PyDreoAC(%s):handle_server_update - mode: %s --> %s", self, self._mode, target_mode)
            self._mode = target_mode

        val_fan_mode = self.get_server_update_key_value(message, WINDLEVEL_KEY)
        if isinstance(val_fan_mode, int):
            _LOGGER.debug("PyDreoAC(%s):handle_server_update - fan_mode: %s --> %s",
                          self, self._fan_mode, val_fan_mode)
        ...
        val_ctlstatus = self.get_server_update_key_value(
            message, CTLSTATUS_KEY)
        ...
        val_childlockon = self.get_server_update_key_value(
            message, CHILDLOCKON_KEY)
        if isinstance(val_childlockon, bool):
            self._childlockon = val_childlockon

        val_tempoffset = self.get_server_update_key_value(
            message, TEMPOFFSET_KEY)
